...
class AeDeepModel(BaseDeepModel):
    def __init__(self, train_params, weights_dir, gpus=1):
        super(AeDeepModel, self).__init__(train_params, gpus)
        self.weights_path = weights_dir

    def build_model(self,input_shape,opt=None,loss_fn=None):
        self.input_shape = input_shape
        self.model = AeDeepModel.get_conv_autoencoder_model(input_shape)
        if opt:
            if self.gpus <= 1:
                self.model.summary()
